...
2. **Regarding Security Flags:** Since this is an unsigned tool that modifies system firewall rules, Windows Defender may flag it as a threat. **I have open-sourced 100% of the code.** If you decide to use this tool, please **review the source code yourself**. Once you are satisfied with its integrity, you can manually "Allow on device" to let it run.
3. Double-click and grant Administrator privileges (Required to modify Windows Firewall rules).
4. **Zero Configuration Needed:** You don't need to do anything for apps you want to allow.
5. **Target & Kill:** Simply find the app you want to silence, click the **Red Button**, and it will automatically drop to the bottom of the list and stay blocked. Keep everything else as it is.
